...
class BlueTeamEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # AISLAR
            ...
            if nodo['aislado'] == 0:
                ...
                if nodo['estado'] == 1:
                    ...
                    # Aislar nodo sano: penalización proporcional
                    if nodo['analizado'] == 1:
        ...
        if self.paso_red_team >= len(self.playbook_actual):
            # El playbook ha terminado. Comprobamos si Blue ganó o si Red consumó su ataque
            crown_jewels = (6, 8, 9)
            cj_cifrados = sum(1 for n in crown_jewels if self.red.nodes[n]['cifrado'] == 1)
            
            if cj_cifrados == 0:
                reward += 300  # Victoria total del Blue
                info['outcome'] = 'victoria_blue'
            elif cj_cifrados < 3:
                reward += 100  # Contención parcial
                info['outcome'] = 'contencion_parcial'
            else:
                reward -= 200  # Red consumó el ataque
                info['outcome'] = 'derrota_blue_cj'
            ...
            if verbo_red == "vuln":
                if nodo_red['aislado'] == 1:
                    ...
                    if nodo_red['nombre'] == "Domain_Controller":
                        # DC vulnerado, pero no es derrota inmediata (el Red aún tiene que cifrarlo)
                        reward -= 100

            # CIFRAR
            elif verbo_red == "lock":
                if nodo_red['honeyfile'] == 1 and random.random() >= 0.35:
                    # Red NO detecta el honeyfile, cae en la trampa
                    reward += 150
                    nodo_red['nivel_alerta'] = 4
                elif nodo_red['honeyfile'] == 1:
                    # Red detecta el honeyfile y simplemente avanza sin efecto
                    pass
...
if __name__ == "__main__":
    ...
    for seed in SEEDS:
        ...
        log_dir = f"logs_blue/seed_{seed}"
        model_dir = f"modelos_blue/seed_{seed}"
